...
<body>
    ...
    <div id="scoreboard" class="overlay-text <?php
        if(isset($_GET["scoreboard"])){
            echo 'visible';
        }
    ?>">
        <?php
            if(isset($_GET["scoreboard"]) && isset($_GET["game"])){

                $scoreTable = GetScoreboard($conn, $_GET["game"]);
                
                $tableScore = array();

                $userName = $_GET['name'];
                $userScore = $_GET['scoreboard'];
                $usersUid = $_SESSION['useruid'];

                $output = '<span class="scoreSpan">';
                $output .= $_GET["game"];
                $output .= '</br>SCOREBOARD</span>
                <div id="scoreb">
            ...
            while($row = mysqli_fetch_assoc($scoreTable)){
                if($userName==$row['usersName'] && $row['usersUid'] == $usersUid)
                {
                    if($test){
                        continue;
                    }else{
                ...
                $output .='</td><td>';
                $output .=$row['gamesScore'];
                $output .='</td></tr>';
            }
            $output .= '</tbody>
        </table></div><div id="myScore">';
        $output .='<table class="styled-table"><thead>
                <tr>
                    <th>YOUR</th>
                    <th></th>
                    <th>SCORE</th>
        ...
        ?>
        <form action="../Script/newGame.inc.php" method="POST" style="min-width=100%; min-height: 10%">
            <input name="game" type="text" value="<?php echo $_GET['game']; ?>" style="display:none;" />
            <button class="newGame newGame1" type="submit" name="submit">New Game</button>
        </form>
