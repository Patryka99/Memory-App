...
<body>
    ...
    <div id="Content">
        <div id="startGameInfo">
            <div id="howToPlay">
                <?php
                    $tag1 = "<h2>How to play</h2>";
                    $tag2 = "<h2>Control</h2>";
                    $crossword = '<span class="playInfo">On bottom of screen You will have six letters and crossword above. You need to 
                    arrange words from those letters and type it on keyboard.
                    Goal is to complete all 10 words in crossword, but You have only 200 sec to do this so think fast.</span>';
                    $simon = '<span class="playInfo">Its something like classic Simon Says.</br> At screen You will have 9 circles. Those circles 
                    will highlights for a second u need to remember witch lights up then reproduce the pattern.</br>
                    Be carefully each round is getting harder.</span>';
                    $slidePuzzle = "<span class='playInfo'>What a mess, someone messed up the puzzle</br>
                    Are you able to sort it out? But its can't be so easy, You can only slide them.</span>";

                    $crosswordControl = "<table class='controlTable'><tr><th>Letters on keyboard</th><td> Type word, but You can use only letters from screen</td></tr>
                    <tr><th>Space</th> <td>Confirm word (Check if it's correct)</td></tr>
                    <tr><th>Enter</th> <td>Skip level if u cant find more words (only 3 skips per game)</td></tr>
                    <tr><th>Backspace</th> <td>Clear letter if u made a mistake</br></td></tr></table>";

                    $simonControl = "<span class='playInfo'>Control is very Simply just click with your left mouse button on Circles </span>";

                    $slideControl ="<table class='controlTable'><tr><th>Left Arrow</th><td> Move block to left (only if empty is on right)</td></tr>
                    <tr><th>Right Arrow</th> <td>Move block to right (only if empty is on left)</td></tr>
                    <tr><th>Up Arrow</th> <td>Move block Up (only if empty is Bottom)</td></tr>
                    <tr><th>Down Arrow</th> <td>Move block Down (only if empty is Up)</br></td></tr></table>";

                    echo $tag1;

                    if($_GET['game'] == 'Crossword')
                    {
                        echo $crossword;
                    }else if($_GET['game'] == 'Simon')
                    {
                        echo $simon;
                    }else if($_GET['game'] == 'SlidePuzzle')
                    {
                        echo $slidePuzzle;
                    }
                    echo $tag2;
            ...
            <div id="choseDif">
                <form name="Play" action="../Script/startGame.inc.php" method="POST">
                    ...
                    <h2>START GAME</h2>
                    <div name="PLAY" class="playBtn btn btn-one" onClick="document.forms['Play'].submit();">
                        <span>PLAY</span></div>
                </form> 
            </div>
        </div>
